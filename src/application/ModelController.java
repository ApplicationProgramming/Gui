...
public class ModelController extends Main{
	...
	public void Login(ActionEvent event) {
		...
		checkLogin(EnteredEmail, EnteredPassword);
	...
	try {
		...
		//Connection conn = DriverManager.getConnection("jdbc:mysql://35.196.80.119:3306/FlightProject","root","MyTeam");
			...
			PreparedStatement pst = (PreparedStatement) conn.prepareStatement("INSERT INTO Login(CustomerEmail, Password, SecurityQuestion, SecurityQuestionAnswer) VALUES (?,?,?,?)");
			PreparedStatement ps =(PreparedStatement) conn.prepareStatement("insert into Customer(CustomerSSN, CustomerFirst, CustomerLast, CustomerStreet, CustomerCity, CustomerZip,CustomerState, CustomerEmail) values(?,?,?,?,?,?,?,?)");
